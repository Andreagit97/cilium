...
var errNodeNotOnSameL2 = errors.New("node is not on the same L2")
...
func (h *Handler) getNodeRoutes(nodeName string, nodeIP net.IP, podCIDRs []netip.Prefix) ([]routeReconciler.DesiredRoute, error) {
	...
	if err != nil {
		if !errors.Is(err, errNodeNotOnSameL2) {
			return nil, err
		}
		if h.cfg.DirectRoutingSkipUnreachable {
			h.logger.Debug(
				"route to destination contains gateway, skipping route as not directly reachable",
				logfields.NodeName, nodeName,
				logfields.Message, err.Error(),
			)
			return nil, nil
		} else {
			return nil, fmt.Errorf("%w. Add `direct-routing-skip-unreachable` to skip unreachable routes.", err)
		}
	}

	// We now get the device associated with the index.
